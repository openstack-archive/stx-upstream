...
function export_file_from_rbd_image {
    ...
    if [ $? -ne 0 ]; then
    echo "Error: Failed to export image ${file} from Ceph images pool,\
        please check status of storage cluster"
    remove_tmp; exit 1
    fi
}
...
function export_image {
    ...
    if [ $? -ne 0 ]; then
    echo "Error: Corresponding file for image with id: \
        ${uuid} was not found in the RBD images pool"
    remove_tmp; exit 1
    fi
...
function import_file_to_rbd_image {
    ...
    if [ $? -ne 0 ]; then
        echo "Error: Failed to import image ${file} into Ceph images pool,\
        please check status of storage cluster"
        remove_tmp; exit 1
    fi
    rbd snap create ${snap} 1>/dev/null
    if [ $? -ne 0 ]; then
        echo "Error: Failed to create snapshot ${snap},\
        please check status of storage cluster"
        remove_tmp; exit 1
    fi
    rbd snap protect ${snap} 1>/dev/null
    if [ $? -ne 0 ]; then
        echo "Error: Failed to protect snapshot ${snap},\
        please check status of storage cluster"
        remove_tmp; exit 1
    fi
}
...
if [ $# -ne 2 ]; then
    ...
    if [ $? -ne 0 ]; then
        ...
        echo "This command should be used only on \
        setups with configured Ceph storage."
        exit 1
    fi
    ...
    if [ "$1" = "export" ]; then
        ...
        if [ $? -ne 0 ]; then
            echo "Error: Glance image with id: $2 not found. \
            Please try with an existing image id."
            remove_tmp; exit 1
        fi

        # Only allow backup of images that use rbd as backend.
        glance image-show $2 | grep 'direct_url' \
        | awk '{print $4}' | grep -q '^rbd://'
        if [ $? -ne 0 ]; then
            echo "Image with id: $2 is not stored in Ceph RBD.\
                Backup using image-backup tool is not needed."
        ...
        if [ $? -ne 0 ]; then
            echo "Error: Glance image with id: ${uuid} not found. \
            Please try with an existing image id."
            exit 1
        fi
